...
test('documents the provider-neutral AI workspace handoff', () => {
  ...
  assert.match(readme, /\.\/run\.sh dev 3107/);
  assert.match(readme, /run\.ps1 dev 3107/);
...
test('documents and exposes Gofer app test script aliases', () => {
  ...
  assert.match(
    packageJson.scripts['test:business-scenarios'],
...
test('includes real browser tests for Gofer release gates', async () => {
  await access(
    new URL('./business-scenarios/starter-app.spec.ts', import.meta.url),
  );
